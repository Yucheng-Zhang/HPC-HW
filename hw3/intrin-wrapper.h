...
public:
  ...
  Vec operator~() const {
    ...
    for (int i = 0; i < (int)(N * sizeof(ValueType)); i++)
      vo[i] = ~vi[i];
    return r;
  }
  ...
  friend Vec operator<(Vec lhs, const Vec &rhs) {
    ...
    for (int i = 0; i < N; i++)
      lhs.v[i] = (lhs.v[i] < rhs.v[i] ? value_one : value_zero);
    return lhs;
  }
  friend Vec operator<=(Vec lhs, const Vec &rhs) {
    static const ValueType value_zero = const_zero();
  ...
  friend Vec operator&(Vec lhs, const Vec &rhs) {
    ...
    for (int i = 0; i < (int)sizeof(ValueType) * N; i++)
      vo[i] &= vi[i];
    return lhs;
  }
  friend Vec operator^(Vec lhs, const Vec &rhs) {
  ...
  friend Vec AndNot(Vec lhs, const Vec &rhs) { return lhs & (~rhs); }
  ...
  friend std::ostream &operator<<(std::ostream &os, const Vec &in) {
    // for (int i = 0; i < (int)sizeof(ValueType)*8; i++) os <<
    // ((*(uint64_t*)in.v) & (1UL << i) ? '1' : '0'); os << '\n';
    for (int i = 0; i < N; i++)
      os << in.v[i] << ' ';
    return os;
  }
...
private:
  static const ValueType const_zero() {
    ...
    for (int i = 0; i < (int)sizeof(ValueType); i++)
      cvalue[i] = 0;
    return value;
  }
...
// Other operators
template <class RealVec, class IntVec>
RealVec ConvertInt2Real(const IntVec &x) {
  ...
  union {
    Int Cint = (1UL << (SigBits - 1)) +
               ((SigBits + ((1UL << (sizeof(Real) * 8 - SigBits - 2)) - 1))
                << SigBits);
    Real Creal;
  };
...
public:
  ...
  Vec(const ValueType &a) { v = _mm256_set1_pd(a); }

  void Store(ValueType *p) const { _mm256_storeu_pd(p, v); }
  void StoreAligned(ValueType *p) const { _mm256_store_pd(p, v); }
  ...
  template <class Vec>
  friend void sincos_intrin(Vec &sinx, Vec &cosx, const Vec &x);
